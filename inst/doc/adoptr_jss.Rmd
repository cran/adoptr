...
under \url{https://doi.org/10.18637/jss.v098.i09}.
...
`ExpectedSampleSize`.
Further predefined unconditional scores are `MaximumSampleSize`, 
evaluating the maximum sample size, `N1`, measuring the
first-stage sample size, and `AverageN2`, evaluating the average
of the stage-two sample size (improper prior).
...
`DataDistribution`.
Currently, the three options `Normal`, `Binomial`, and `Student` are implemented.
...
```
The optimal design under the point prior only achieves an expected
power of
```{r case-2-evaluate-epwr-opt1}
evaluate(epower, opt1$design)
```
The optimal design under the truncated normal prior fulfilling
the expected power constraint is then given by
...
This shows that an increased uncertainty on $\theta$ 
...
(see Section [6.1](#sec:standard_case)) is only
`r if (buildvignette) round(evaluate(ess, opt1$design), 1)`.
...
organized around a public GitHub.com repository (https://github.com/optad/adoptr).
...
use of the continuous integration and deployment services GitHub Actions [@ghactions].
Each new commit on the public GitHub.com repository is immediately run through
the automated testing pipeline.
Merges to the main branch are only possible after tests were
...
workflow since changes to the main branch require passing the automated tests.
